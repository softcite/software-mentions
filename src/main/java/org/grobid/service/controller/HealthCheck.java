 ...
 *
 * <p><b>On readiness and lazily loaded models.</b> the models are loaded on the first request
 * that needs them, not at startup, so {@code models.loaded} is legitimately empty on a freshly
 * started service. Readiness therefore does <em>not</em> require any model to be loaded - it
 * requires that none has <em>failed</em>. Reporting a fresh service as unhealthy until someone
 * happens to send a request would make the indicator useless.
 */
@Path("health")
@Singleton
